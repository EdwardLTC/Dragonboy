...
namespace Mod.Graphics
{
	internal class ComboBox
	{
		...
		static readonly int TEXT_GAP_X = 4;
		...
		readonly Command expandListItems;
		readonly int h;
		readonly Image imgTf;
		readonly ScrollableMenuItems<string> scrollableMenuItems;
		...
		internal bool IsShowingListItems { get; private set; }
		...
		internal void Paint(mGraphics g)
		{
			...
			expandListItems.w = expandListItems.img.getWidth();
			expandListItems.h = expandListItems.img.getHeight();
		...
		internal void UpdateKey()
		{
			if (!IsFocus && GameCanvas.isPointerHoldIn(x, y, w, h))
			{
				...
				if (GameCanvas.isPointerSelect && !GameCanvas.isPointerMove && (lastMouseY == GameCanvas.pyMouse || lastMouseY == -1))
				{
			...
			if (!IsFocus)
				return;
			if (GameCanvas.isPointerJustDown && lastMouseY == -1 && IsPointerIn(scrollableMenuItems.X, scrollableMenuItems.Y, scrollableMenuItems.Width, scrollableMenuItems.Height))
