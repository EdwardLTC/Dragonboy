...
namespace Mod.PickMob
{
	public class PickMobController
	{
		...
		public static void Update()
		{
			...
			if (Pk9rPickMob.IsAutoPickItems && !isTanSatTDLT)
			{
				...
				if (IsPickingItems)
				{
					...
					switch (GetTypePickItem(itemMap))
					{
				...
				for (int i = 0; i < GameScr.vItemMap.size(); i++)
				{
					...
					if (GetTypePickItem(itemMap) != TypePickItem.CanNotPickItem) ItemPicks.Add(itemMap);
				}
		...
		static bool IsSkillBetter(Skill SkillBetter, Skill skill)
		{
			if (SkillBetter == null)
				return false;
