...
namespace Mod
{
	internal static class ExtensionMethods
	{
		...
		internal static int GetTimeStone(this Char ch)
		{
			return 5;
		}

		internal static int GetTimeWhistle(this Char ch)
		{
			return 31;
		}

		internal static int GetTimeChocolate(this Char ch)
		{
			return 31;
		}

		internal static int GetTimeSelfExplode(this Char ch)
		{
			return 3;
		}

		internal static int GetTimeQCKK(this Char ch)
		{
			return 3;
		}
		...
		internal static string GetClanTag(this Char ch)
		{
		...
		internal static bool IsNormalChar(this Char ch, bool isIncludeBoss = false, bool isIncludePet = false)
		{
			...
			{
				if (!isIncludeBoss)
					result = result && !char.IsUpper(ch.GetNameWithoutClanTag()[0]);
				if (!isIncludePet)
		...
		internal static bool IsBoss(this Char ch)
		{
			return !ch.IsPet() && ch.cName != LocalizedString.arbitration && char.IsUpper(ch.GetNameWithoutClanTag()[0]);
		}

		internal static bool IsPet(this Char ch)
		{
		...
		internal static Char ClosestChar(this Char ch, int maxDistance, bool isNormalCharOnly)
		{
			...
			for (int i = 0; i < GameScr.vCharInMap.size(); i++)
			{
				...
				if (c == ch)
					continue;
				if (isNormalCharOnly && !c.IsNormalChar())
					continue;
		...
		internal static bool CanUse(this Skill skill)
		{
			...
			bool isOutOfCooldown = now - skill.lastTimeUseThisSkill > skill.coolDown;
			return isOutOfCooldown && skill.HasManaToUseSkill();
		}
		...
		internal static int GetPetId(this Char ch)
		{
			return -ch.charID;
		}
		...
		{
			int margin = TileMap.size * 2;

			return waypoint.maxX < margin ? margin :
				waypoint.minX > TileMap.pxw - margin ? TileMap.pxw - margin :
				waypoint.minX + (waypoint.maxX - waypoint.minX) / 2;
		}
