...
namespace Mod.Xmap
{
	public class EdwardXmap : IActionListener
	{
		public class NextMap
		{
			...
			public int Index;
			bool isEntering;
			long enterDelayStart;
			Waypoint enterPendingWaypoint;
			bool isTeleNpc;
			long teleNpcStartTime;
			public int Index2;
			public int Index3;
			public bool walk;
			public int x;
		...
		static EdwardXmap _Instance;

		static Dictionary<int, List<NextMap>> linkMaps;

		static Dictionary<string, int[]> planetDictionary;

		static bool isXmaping;

		static int IdMapEnd;

		static int[] wayPointMapLeft;

		static int[] wayPointMapCenter;

		static int[] wayPointMapRight;

		static bool isEatChicken;

		static bool isHarvestPean;

		static bool isUseCapsule;

		static bool isUsingCapsule;

		static bool isOpeningPanel;

		static long lastTimeOpenedPanel;

		static bool isSaveData;

		static long lastWaitTime;

		static int[] idMapsNamek;

		static int[] idMapsXayda;

		static int[] idMapsTraiDat;

		static int[] idMapsTuongLai;

		static int[] idMapsCold;

		static int[] idMapsNappa;

		static int[] idMapsThapleo;

		static int[] idMapsManhVoBT;

		static int[] idMapsKhiGas;

		static long lastErrorTime;

		static long lastItemUseTime;

		static bool isUsingItem;

		public static bool xmapErrr;

		public static EdwardXmap getInstance()
		{
			if (_Instance == null)
			{
				_Instance = new EdwardXmap();
			}
			return _Instance;
		}
		...
		public static void UpdateXmap(int mapID)
		{
			...
			linkMaps[999].Add(new NextMap(24 + Char.myCharz().cgender, 10, 0, -1, -1, false, -1, -1));
			if (IdMapEnd == 160 && !isUsingItem)
			{
				if (Utils.getItemInBag(992) == null)
				{
		...
		static void LoadNPCLinkMapsXmap()
		{
			...
			AddNPCLinkMapsXmap(155, 166, -1, -1, -1, -1, true, 1400, 600);
			AddNPCLinkMapsXmap(46, 47, -1, -1, -1, -1, true, 80, 700);
			AddNPCLinkMapsXmap(45, 46, -1, -1, -1, -1, true, 80, 700);
			AddNPCLinkMapsXmap(46, 45, -1, -1, -1, -1, true, 380, 90);
		...
		static void AddLinkMapsXmap(params int[] link)
		{
			for (int i = 0; i < link.Length; i++)
			{
				...
				if (i != 0)
				{
					linkMaps[link[i]].Add(new NextMap(link[i - 1], -1, -1, -1, -1, false, -1, -1));
				}
				if (i != link.Length - 1)
				{
					linkMaps[link[i]].Add(new NextMap(link[i + 1], -1, -1, -1, -1, false, -1, -1));
		...
		static bool hasWayGoFutureAndBack(int[] ways)
		{
		...
		static bool hasWayGoToColdMap(int[] ways)
		{
		...
		static string GetMapName(int mapID)
		{
			string text = mapID switch
			{
				...
				113 => string.Concat(new object[3]
				{
					"Siêu hạng\n[", mapID, "]"
				}),
		...
		static bool isNRDMap(int mapID)
		{
			if (mapID >= 85)
			{
				return mapID <= 91;
		...
		static bool isNRD(ItemMap mapID)
		{
		...
		static EdwardXmap()
		{
			...
			idMapsThapleo = new int[7]
			{
				46, 45, 48, 50, 154, 155, 166
			};
			idMapsManhVoBT = new int[5]
			{
				153, 156, 157, 158, 159
			};
			idMapsKhiGas = new int[5]
			{
				149, 147, 152, 151, 148
			};
			LoadLinkMapsXmap();
		...
		static void AddNPCLinkMapsXmap(int currentMapID, int nextMapID, int npcID, int select, int select2 = -1, int select3 = -1, bool walk = false, int x = -1, int y = -1)
		{
		...
		static string GetErrorMessage(int mapID, List<int[]> validPaths, long currentPower)
		{
			bool flag = false;
			string result = "";
			...
			if (!flag && mapID == 160 && Utils.getItemInBag(992) == null)
			{
