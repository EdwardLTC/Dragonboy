...
namespace Mod.Auto
{
    ...
    {
        internal static List<int> mobTemplateIdList = new();

        internal static List<int> mobIdList = new();
        ...
        private static KeyValuePair<int, int> mapZoneGoBack;
        ...
        {
            Mode = (AutoTrainPetMode)value;
        }

        internal static void SetAttackState(int value)
        {
            ModeAttackWhenNeeded = (AutoTrainPetAttackMode)value;
        }
        ...
        internal static void Update()
        {
            ...
            if (GameCanvas.gameTick % (30 * Time.timeScale) != 0 || XmapController.gI.IsActing)
                return;
            AutoPick();
            if (Char.myPetz().cStamina < 5 && GameScr.hpPotion > 0 && !(Char.myPetz().cHP <= 0 || Char.myPetz().isDie))
                GameScr.gI().doUseHP();
            ...
            if (isGoHomeGetMorePean && !isMagicTreeOutOfPean && GameCanvas.gameTick % (60 * Time.timeScale) == 0)
            {
                ...
                else
                {
                    ...
                    if ((GameScr.gI().magicTree.isPeasEffect || GameScr.gI().magicTree.isUpdateTree ||
                         GameScr.gI().magicTree.currPeas == 0) && !XmapController.gI.IsActing)
            ...
            if (mSystem.currentTimeMillis() - lastTimePetFollow > 600000 ||
                (GameScr.findCharInMap(-Char.myCharz().charID) != null &&
                 Utils.Distance(Char.myCharz(), GameScr.findCharInMap(-Char.myCharz().charID)) > 400))
            {
        ...
        {
            if (!isPicking &&
                (Char.myCharz().arrItemBody[5] == null || Char.myCharz().arrItemBody[5].template.id != 449))
            {
                var myPet = GameScr.findCharInMap(-Char.myCharz().charID);
                if (myPet == null)
                    myPet = Char.myPetz();
                if (myPet != null)
                {
        ...
        {
            //auto skill 3
            var skill3 = Char.myCharz().getSkill(Char.myCharz().nClass.skillTemplates[2]);
            var skill1 = Char.myCharz().getSkill(Char.myCharz().nClass.skillTemplates[0]);
            if (skill3 != null)
            {
                if (skill3.point > 0 && mSystem.currentTimeMillis() - skill3.lastTimeUseThisSkill > skill3.coolDown)
                {
                    if ((Char.myPetz().cHP * 100 / Char.myPetz().cHPFull < 10 ||
                         Char.myPetz().cMP * 100 / Char.myPetz().cMPFull < 10 ||
                         Char.myCharz().cHP * 100 / Char.myCharz().cHPFull < 10 ||
                         Char.myCharz().cMP * 100 / Char.myCharz().cMPFull < 10) && Char.myCharz().cgender == 1 &&
                        skill3.manaUse < Char.myCharz().cMP)
                    {
                        if (skill3.point > 1)
                        {
                            Utils.buffMe();
                    ...
                    if ((Char.myCharz().cHP * 100 / Char.myCharz().cHPFull < 10 ||
                         Char.myCharz().cMP * 100 / Char.myCharz().cMPFull < 10) && Char.myCharz().cgender == 2)
                    {
                ...
                if (Char.myCharz().cgender == 2 && isTTNL &&
                    mSystem.currentTimeMillis() - skill3.lastTimeUseThisSkill > 10000)
                {
            ...
            if (controlSkill != null)
                ...
                {
                    var superMob = (Mob)GameScr.vMob.elementAt(i);
        ...
        private static Mob ClosestMob()
        {
            ...
            for (var i = 0; i < GameScr.vMob.size(); i++)
            {
                ...
                if (mob.status != 0 && mob.status != 1 && mob.hp > 0 && !mob.isMobMe && mob.levelBoss == 0 &&
                    mob.getTemplate().type != MonsterType.Fly &&
                    ((mobTemplateIdList.Count > 0 && mobTemplateIdList.Contains(mob.templateId)) ||
                     (mobIdList.Count > 0 && mobIdList.Contains(mob.mobId)) || mobTemplateIdList.Count == 0 ||
                     mobIdList.Count == 0))
                {
                    var distance = Res.distance(mob.x, mob.y, Char.myCharz().cx, Char.myCharz().cy);
        ...
        {
            if (!Pk9rPickMob.IsAutoPickItems) return;
            if (mSystem.currentTimeMillis() - lastTimePick > 550)
            {
                var hasPickableItem = false;
                if (GameScr.vItemMap.size() == 0)
                    isPicking = false;
                for (var i = GameScr.vItemMap.size() - 1; i >= 0; i--)
                {
                    var itemMap = (ItemMap)GameScr.vItemMap.elementAt(i);
                    if ((itemMap.template.id >= 828 && itemMap.template.id <= 842) || itemMap.template.id == 859 ||
                        itemMap.template.id == 362 || (itemMap.template.id >= 353 && itemMap.template.id <= 360))
                    {
                    ...
                    var distance = Utils.Distance(itemMap, Char.myCharz());
