...
public class GameScr : mScreen, IChatable
{
    ...
    internal static Scroll scrInfo = new();
    ...
    public static MyVector res_CT = new();
    ...
    public static void loadBg()
    {
        ...
        for (var i = 0; i < 5; i++) flyTextState[i] = -1;
        var array = Rms.loadRMS("NRdataVersion");
        var array2 = Rms.loadRMS("NRmapVersion");
        var array3 = Rms.loadRMS("NRskillVersion");
        var array4 = Rms.loadRMS("NRitemVersion");
        if (array != null)
            vcData = array[0];
        if (array2 != null)
            vcMap = array2[0];
    ...
    public static void paintOngMauPercent(Image img0, Image img1, Image img2, float x, float y, int size,
        float pixelPercent, mGraphics g)
    {
        var clipX = g.getClipX();
        var clipY = g.getClipY();
        var clipWidth = g.getClipWidth();
        var clipHeight = g.getClipHeight();
        g.setClip((int)x, (int)y, (int)pixelPercent, 13);
        var num = size / 15 - 2;
        for (var i = 0; i < num; i++) g.drawImage(img1, x + (i + 1) * 15, y, 0);
    ...
    {
        var num = 0;
        for (var i = 0; i <= level; i++) num += (int)exps[i];
        return num;
    }
    ...
    public void onOSkill(sbyte[] oSkillID)
    {
        ...
        for (var i = 0; i < oSkillID.Length; i++)
        for (var j = 0; j < Char.myCharz().vSkillFight.size(); j++)
        {
            var skill = (Skill)Char.myCharz().vSkillFight.elementAt(j);
    ...
    public void onKSkill(sbyte[] kSkillID)
    {
        ...
        for (var i = 0; i < kSkillID.Length; i++)
        for (var j = 0; j < Char.myCharz().vSkillFight.size(); j++)
        {
            var skill = (Skill)Char.myCharz().vSkillFight.elementAt(j);
    ...
    public void onCSkill(sbyte[] cSkillID)
    {
        ...
        {
            for (var i = 0; i < Char.myCharz().vSkillFight.size(); i++)
            {
                var skill = (Skill)Char.myCharz().vSkillFight.elementAt(i);
    ...
    internal void loadDefaultonScreenSkill()
    {
        ...
        for (var i = 0; i < onScreenSkill.Length && i < Char.myCharz().vSkillFight.size(); i++)
        {
            var skill = (Skill)Char.myCharz().vSkillFight.elementAt(i);
            onScreenSkill[i] = skill;
        }
    ...
    internal void loadDefaultKeySkill()
    {
        ...
        for (var i = 0; i < keySkill.Length && i < Char.myCharz().vSkillFight.size(); i++)
        {
            var skill = (Skill)Char.myCharz().vSkillFight.elementAt(i);
            keySkill[i] = skill;
        }
    ...
    {
        var skill = Char.myCharz().getSkill(skillTemplate);
        ...
        for (var i = 0; i < 10; i++)
        {
            ...
            var command = new Command(mResources.into_place + (i + 1), 11120, p);
    ...
    public void doSetKeySkill(SkillTemplate skillTemplate)
    {
        ...
        var skill = Char.myCharz().getSkill(skillTemplate);
        var array = !TField.isQwerty ? mResources.key_skill : mResources.key_skill_qwerty;
        var myVector = new MyVector();
        for (var i = 0; i < 10; i++)
        {
            var p = new object[2]
    ...
    {
        var array = new sbyte[onScreenSkill.Length];
        for (var i = 0; i < onScreenSkill.Length; i++)
            if (onScreenSkill[i] == null)
                array[i] = -1;
            else
    ...
    {
        var array = new sbyte[keySkill.Length];
        for (var i = 0; i < keySkill.Length; i++)
            if (keySkill[i] == null)
                array[i] = -1;
            else
    ...
    public void addSkillShortcut(Skill skill)
    {
        ...
        for (var j = 0; j < keySkill.Length; j++)
            if (keySkill[j] == null)
            {
                keySkill[j] = skill;
                break;
    ...
    {
        for (var num = Char.myCharz().arrItemBag.Length - 1; num >= 0; num--)
    ...
    {
        var myVector = new MyVector();
        for (var i = 0; i < menu.Length; i++) myVector.addElement(new Command(menu[i], 11057, npc));
    ...
    public void readDart()
    {
        ...
        try
        {
            ...
            for (var i = 0; i < num; i++)
            {
                ...
                for (var j = 0; j < num2; j++) darts[i].tail[j] = dataInputStream.readShort();
                num2 = dataInputStream.readShort();
                darts[i].tailBorder = new short[num2];
                for (var k = 0; k < num2; k++) darts[i].tailBorder[k] = dataInputStream.readShort();
                num2 = dataInputStream.readShort();
                darts[i].xd1 = new short[num2];
                for (var l = 0; l < num2; l++) darts[i].xd1[l] = dataInputStream.readShort();
                num2 = dataInputStream.readShort();
                darts[i].xd2 = new short[num2];
                for (var m = 0; m < num2; m++) darts[i].xd2[m] = dataInputStream.readShort();
                num2 = dataInputStream.readShort();
                darts[i].head = new short[num2][];
                for (var n = 0; n < num2; n++)
                {
                    var num3 = dataInputStream.readShort();
                    darts[i].head[n] = new short[num3];
                    for (var num4 = 0; num4 < num3; num4++) darts[i].head[n][num4] = dataInputStream.readShort();
                }

                num2 = dataInputStream.readShort();
                darts[i].headBorder = new short[num2][];
                for (var num5 = 0; num5 < num2; num5++)
                {
    ...
    public void readSkill()
    {
        ...
        try
        {
            ...
            for (var i = 0; i < num; i++)
            {
                var num2 = dataInputStream.readShort();
                if (num2 == 1111)
                    num2 = (short)(num - 1);
                ...
                for (var j = 0; j < sks[num2].skillStand.Length; j++)
                {
                ...
                for (var k = 0; k < sks[num2].skillfly.Length; k++)
                {
    ...
    {
        var array = !GameCanvas.isTouch ? keySkill : onScreenSkill;
    ...
    {
        var clanObject = new ClanObject();
        clanObject.code = code;
        clanObject.clanID = clanID;
        startYesNoPopUp(strInvite, new Command(mResources.YES, 12002, clanObject),
            new Command(mResources.NO, 12003, clanObject));
    }
    ...
    public void playerMenu(Char c)
    {
        ...
        if (Char.myCharz().charFocus.charID < 0 || Char.myCharz().charID < 0)
            return;
        var vPlayerMenu = GameCanvas.panel.vPlayerMenu;
        ...
        if (Char.myCharz().clan != null && Char.myCharz().clan.ID == Char.myCharz().charFocus.clanID &&
            Char.myCharz().charFocus.statusMe != 14 && Char.myCharz().taskMaint != null &&
            Char.myCharz().taskMaint.taskId >= 14)
            vPlayerMenu.addElement(new Command(mResources.CHAR_ORDER[1], 2004));
        var num = Char.myCharz().nClass.skillTemplates.Length;
        for (var i = 0; i < num; i++)
        {
            var skillTemplate = Char.myCharz().nClass.skillTemplates[i];
            var skill = Char.myCharz().getSkill(skillTemplate);
    ...
    public bool isAttack()
    {
        ...
        if (InfoDlg.isLock || Char.myCharz().isLockAttack || Char.isLockKey)
            return false;
        if (Char.myCharz().myskill != null && Char.myCharz().myskill.template.id == 6 &&
        ...
        if (Char.myCharz().myskill != null && Char.myCharz().myskill.template.type == 2 &&
            Char.myCharz().npcFocus == null && Char.myCharz().myskill.template.id != 6)
        {
            if (!checkSkillValid())
                return false;
            return true;
        ...
        if (Char.myCharz().skillPaint != null || (Char.myCharz().mobFocus == null && Char.myCharz().npcFocus == null &&
                                                  Char.myCharz().charFocus == null && Char.myCharz().itemFocus == null))
            return false;
        if (Char.myCharz().mobFocus != null)
        {
            ...
            if (Char.myCharz().isSelectingSkillUseAlone())
                return false;
            var num = -1;
            var num2 = Res.abs(Char.myCharz().cx - cmx) * mGraphics.zoomLevel;
            ...
            if ((Char.myCharz().mobFocus.status == 1 || Char.myCharz().mobFocus.status == 0 ||
                 Char.myCharz().myskill.template.type == 4 || num == -1 || num > num2) &&
                Char.myCharz().myskill.template.type == 4)
            {
            ...
            if (Char.myCharz().cx < Char.myCharz().mobFocus.getX())
                ...
                Char.myCharz().cdir = -1;
            var num3 = Math2.abs(Char.myCharz().cx - Char.myCharz().mobFocus.getX());
            var num4 = Math2.abs(Char.myCharz().cy - Char.myCharz().mobFocus.getY());
            Char.myCharz().cvx = 0;
            if (num3 <= Char.myCharz().myskill.dx && num4 <= Char.myCharz().myskill.dy)
            {
                if (Char.myCharz().myskill.template.id == 20)
                    return true;
                if (num4 > num3 && Res.abs(Char.myCharz().cy - Char.myCharz().mobFocus.getY()) > 30 &&
                    Char.myCharz().mobFocus.getTemplate().type == 4)
                {
                    Char.myCharz().currentMovePoint = new MovePoint(Char.myCharz().cx + Char.myCharz().cdir,
                        Char.myCharz().mobFocus.getY());
                ...
                {
                    var num6 = Char.myCharz().cx > Char.myCharz().mobFocus.getX() ? 1 : -1;
                    if ((TileMap.tileTypeAtPixel(Char.myCharz().mobFocus.getX() + num5 * num6, Char.myCharz().cy + 3) &
                         2) != 2)
                        flag2 = true;
                }
                ...
                if (num3 <= num5 && !flag2)
                {
                    if (Char.myCharz().cx > Char.myCharz().mobFocus.getX())
                    {
                        var num7 = Char.myCharz().mobFocus.getX() + num5 + (flag ? 30 : 0);
                        var i = Char.myCharz().mobFocus.getX();
                        var flag3 = false;
                        for (; i < num7; i += 24)
                            if (TileMap.tileTypeAtPixel(i, Char.myCharz().cy + 3) == 8 ||
                                TileMap.tileTypeAtPixel(i, Char.myCharz().cy + 3) == 4)
                            {
                                flag3 = true;
                                break;
                            }
                    ...
                    {
                        var num8 = Char.myCharz().mobFocus.getX() - num5 - (flag ? 30 : 0);
                        var num9 = Char.myCharz().mobFocus.getX();
                        var flag4 = false;
                        while (num9 > num8)
                        {
                            if (TileMap.tileTypeAtPixel(num9, Char.myCharz().cy + 3) == 8 ||
                                TileMap.tileTypeAtPixel(num9, Char.myCharz().cy + 3) == 4)
                            {
                                flag4 = true;
                                break;
                            }
            ...
            if (Char.myCharz().mobFocus is BigBoss || Char.myCharz().mobFocus is BigBoss2)
                flag5 = true;
            var num10 = (Char.myCharz().myskill.dx - (!flag5 ? 20 : 50)) *
                        (Char.myCharz().cx > Char.myCharz().mobFocus.getX() ? 1 : -1);
            if (num3 <= Char.myCharz().myskill.dx)
                num10 = 0;
            Char.myCharz().currentMovePoint =
                new MovePoint(Char.myCharz().mobFocus.getX() + num10, Char.myCharz().mobFocus.getY());
            Char.myCharz().endMovePointCommand = new Command(null, null, 8002, null);
            GameCanvas.clearKeyHold();
            GameCanvas.clearKeyPressed();
            return false;
        ...
        if (Char.myCharz().npcFocus != null)
        {
            ...
            if (Char.myCharz().cx < Char.myCharz().npcFocus.cx)
                ...
                Char.myCharz().npcFocus.cdir = 1;
            var num11 = Math2.abs(Char.myCharz().cx - Char.myCharz().npcFocus.cx);
            ...
            {
                var num12 = 20 + Res.r.nextInt(20);
                var num13 = Char.myCharz().cx > Char.myCharz().npcFocus.cx ? 1 : -1;
                Char.myCharz().currentMovePoint =
                    new MovePoint(Char.myCharz().npcFocus.cx + num12 * num13, Char.myCharz().cy);
                Char.myCharz().endMovePointCommand = new Command(null, null, 8002, null);
                GameCanvas.clearKeyHold();
                GameCanvas.clearKeyPressed();
            }
        ...
        if (Char.myCharz().charFocus != null)
        {
            ...
            if (Char.myCharz().cx < Char.myCharz().charFocus.cx)
                ...
                Char.myCharz().cdir = -1;
            var num14 = Math2.abs(Char.myCharz().cx - Char.myCharz().charFocus.cx);
            var num15 = Math2.abs(Char.myCharz().cy - Char.myCharz().charFocus.cy);
            if (Char.myCharz().isMeCanAttackOtherPlayer(Char.myCharz().charFocus) ||
                Char.myCharz().isSelectingSkillBuffToPlayer())
            {
                if (Char.myCharz().myskill == null)
                    return false;
                if (!checkSkillValid())
                ...
                if (num14 <= Char.myCharz().myskill.dx && num15 <= Char.myCharz().myskill.dy)
                {
                    ...
                    if ((TileMap.tileTypeAtPixel(Char.myCharz().cx, Char.myCharz().cy + 3) & 2) == 2)
                    {
                        var num17 = Char.myCharz().cx > Char.myCharz().charFocus.cx ? 1 : -1;
                        if ((TileMap.tileTypeAtPixel(Char.myCharz().charFocus.cx + num16 * num17,
                                Char.myCharz().cy + 3) & 2) != 2)
                            flag6 = true;
                    }
                ...
                var num18 = (Char.myCharz().myskill.dx - 20) *
                            (Char.myCharz().cx > Char.myCharz().charFocus.cx ? 1 : -1);
                if (num14 <= Char.myCharz().myskill.dx)
                    num18 = 0;
                Char.myCharz().currentMovePoint =
                    new MovePoint(Char.myCharz().charFocus.cx + num18, Char.myCharz().charFocus.cy);
                Char.myCharz().endMovePointCommand = new Command(null, null, 8002, null);
                GameCanvas.clearKeyHold();
                GameCanvas.clearKeyPressed();
                return false;
            ...
            if (num14 < 60 && num15 < 40)
            {
                ...
                if (!GameCanvas.isTouch && Char.myCharz().charFocus.charID >= 0 && TileMap.mapID != 51 &&
                    TileMap.mapID != 52 && popUpYesNo == null)
                {
            ...
            {
                var num19 = 20 + Res.r.nextInt(20);
                var num20 = Char.myCharz().cx > Char.myCharz().charFocus.cx ? 1 : -1;
                Char.myCharz().currentMovePoint = new MovePoint(Char.myCharz().charFocus.cx + num19 * num20,
                    Char.myCharz().charFocus.cy);
                Char.myCharz().endMovePointCommand = new Command(null, null, 8002, null);
                GameCanvas.clearKeyHold();
                GameCanvas.clearKeyPressed();
            }
    ...
    public bool isMeCanAttackMob(Mob m)
    {
        ...
        if (Char.myCharz().mobMe != null && m.Equals(Char.myCharz().mobMe))
            return false;
        var @char = findCharInMap(m.mobId);
        if (@char == null)
            return true;
        if (@char.cTypePk == 5)
            return true;
    ...
    {
        if (Char.myCharz().myskill != null &&
            ((Char.myCharz().myskill.template.manaUseType != 1 &&
              Char.myCharz().cMP < Char.myCharz().myskill.manaUse) ||
             (Char.myCharz().myskill.template.manaUseType == 1 &&
              Char.myCharz().cMP < Char.myCharz().cMPFull * Char.myCharz().myskill.manaUse / 100)))
        {
        ...
        if (Char.myCharz().myskill == null ||
            (Char.myCharz().myskill.template.maxPoint > 0 && Char.myCharz().myskill.point == 0))
        {
    ...
    {
        if (Controller.isStopReadMessage || Char.myCharz().isTeleport || Char.myCharz().isPaintNewSkill ||
            InfoDlg.isLock)
            return;
        ...
        {
            var cmdNextLine = ChatPopup.currChatPopup.cmdNextLine;
        ...
        else
        {
            if (ChatTextField.gI().left != null &&
                (GameCanvas.keyPressed[12] || getCmdPointerLast(ChatTextField.gI().left)) &&
                ChatTextField.gI().left != null)
                ChatTextField.gI().left.performAction();
            if (ChatTextField.gI().right != null &&
                (GameCanvas.keyPressed[13] || getCmdPointerLast(ChatTextField.gI().right)) &&
                ChatTextField.gI().right != null)
                ChatTextField.gI().right.performAction();
            if (ChatTextField.gI().center != null &&
                (GameCanvas.keyPressed[!Main.isPC ? 5 : 25] || getCmdPointerLast(ChatTextField.gI().center)) &&
                ChatTextField.gI().center != null)
        ...
        else
        {
            ...
            if (GameCanvas.keyAsciiPress != 0 && mobCapcha == null && TField.isQwerty &&
                GameCanvas.keyAsciiPress == 121)
            {
                if (popUpYesNo != null)
                {
                    popUpYesNo.cmdYes.performAction();
            ...
            if (GameCanvas.keyAsciiPress != 0)
            {
                ...
                {
                    var array = keyInput.ToCharArray();
                    var myVector = new MyVector();
                    for (var j = 0; j < array.Length; j++) myVector.addElement(array[j] + string.Empty);
                    myVector.removeElementAt(0);
                    var text = (char)GameCanvas.keyAsciiPress + string.Empty;
                    if (text.Equals(string.Empty) || text == null || text.Equals("\n"))
                        text = "-";
                    myVector.insertElementAt(text, myVector.size());
                    keyInput = string.Empty;
                    for (var k = 0; k < myVector.size(); k++) keyInput += ((string)myVector.elementAt(k)).ToUpper();
            ...
            else if (Char.myCharz().statusMe == 2)
            {
                ...
                {
                    if (Char.myCharz().cx - Char.myCharz().cxSend != 0 ||
                        Char.myCharz().cy - Char.myCharz().cySend != 0)
                        Service.gI().charMove();
                    Char.myCharz().cvy = -10;
            ...
            else if (Char.myCharz().statusMe == 3)
            {
                ...
                if ((GameCanvas.keyHold[!Main.isPC ? 2 : 21] ||
                     ((GameCanvas.keyHold[1] || GameCanvas.keyHold[3]) && mobCapcha == null)) &&
                    Char.myCharz().canFly && Char.myCharz().cMP > 0 && Char.myCharz().cp1 < 8 &&
                    Char.myCharz().cvy > -4)
                {
                    Char.myCharz().cp1++;
                    Char.myCharz().cvy = -7;
            ...
            else if (Char.myCharz().statusMe == 4)
            {
                ...
                if (GameCanvas.keyHold[!Main.isPC ? 2 : 21] && Char.myCharz().cMP > 0 && Char.myCharz().canFly)
                {
                    ...
                    if ((Char.myCharz().cx - Char.myCharz().cxSend != 0 ||
                         Char.myCharz().cy - Char.myCharz().cySend != 0) &&
                        (Res.abs(Char.myCharz().cx - Char.myCharz().cxSend) > 96 ||
                         Res.abs(Char.myCharz().cy - Char.myCharz().cySend) > 24))
                        Service.gI().charMove();
                    Char.myCharz().cvy = -10;
                    Char.myCharz().statusMe = 3;
                    Char.myCharz().cp1 = 0;
    ...
    internal void checkDrag()
    {
        ...
        if (isPointerDowning)
        {
            var num = GameCanvas.px - ptLastDownX;
            var num2 = GameCanvas.py - ptLastDownY;
            if (!isChangingCameraMode &&
                (Res.abs(GameCanvas.px - ptFirstDownX) > 15 || Res.abs(GameCanvas.py - ptFirstDownY) > 15))
    ...
    internal void checkClick()
    {
        ...
        else
        {
            if (checkClickToCapcha())
                return;
            var num = mSystem.currentTimeMillis();
    ...
    internal IMapObject findClickToItem(int px, int py)
    {
        ...
        var num = 0;
        var num2 = 30;
        var array = new MyVector[4] { vMob, vNpc, vItemMap, vCharInMap };
        for (var i = 0; i < array.Length; i++)
        for (var j = 0; j < array[i].size(); j++)
        {
            var mapObject2 = (IMapObject)array[i].elementAt(j);
            if (mapObject2.isInvisible())
                continue;
            if (mapObject2 is Mob)
            {
                var mob = (Mob)mapObject2;
                if (mob.isMobMe && mob.Equals(Char.myCharz().mobMe))
                    continue;
            }

            var x = mapObject2.getX();
            var y = mapObject2.getY();
            var w = mapObject2.getW();
            var h = mapObject2.getH();
            ...
            {
                var num3 = Res.abs(px - x) + Res.abs(py - y);
                if (num3 < num)
                {
                    mapObject = mapObject2;
                    num = num3;
    ...
    {
        var num = GameCanvas.px + cmx;
        var num2 = GameCanvas.py + cmy;
        Char.myCharz().cancelAttack();
        var mapObject = findClickToItem(num, num2);
        if (mapObject != null)
        {
            if (Char.myCharz().isAttacPlayerStatus() && Char.myCharz().charFocus != null &&
                !mapObject.Equals(Char.myCharz().charFocus) && !mapObject.Equals(Char.myCharz().charFocus.mobMe) &&
                mapObject is Char)
            {
                var @char = (Char)mapObject;
            ...
            if (Char.myCharz().skillPaint != null || Char.myCharz().arr != null || Char.myCharz().dart != null ||
                Char.myCharz().skillInfoPaint() != null)
                return false;
    ...
    {
        var num = GameCanvas.px + lastClickCMX;
        var num2 = GameCanvas.py + lastClickCMY;
        var cy = Char.myCharz().cy;
        if (isLockKey)
            return;
        var mapObject = findClickToItem(num, num2);
        if (mapObject != null)
    ...
    {
        var xClick = GameCanvas.px + lastClickCMX;
        var yClick = GameCanvas.py + lastClickCMY;
    ...
    {
        for (var i = 0; i < PopUp.vPopups.size(); i++)
        {
            var popUp = (PopUp)PopUp.vPopups.elementAt(i);
    ...
    internal void checkClickMoveTo(int xClick, int yClick, int index)
    {
        ...
        clickOnTileTop = false;
        Char.myCharz().delayFall = 0;
        var num = !Char.myCharz().canFly || Char.myCharz().cMP <= 0 ? 1000 : 0;
        if (clickToY > Char.myCharz().cy && Res.abs(clickToX - Char.myCharz().cx) < 12)
            return;
        for (var i = 0; i < 60 + num && clickToY + i < TileMap.pxh - 24; i += 24)
    ...
    internal void checkAuto()
    {
        ...
        if (GameCanvas.keyPressed[!Main.isPC ? 2 : 21] || GameCanvas.keyPressed[!Main.isPC ? 4 : 23] ||
            GameCanvas.keyPressed[!Main.isPC ? 6 : 24] || GameCanvas.keyPressed[1] || GameCanvas.keyPressed[3])
        {
            auto = 0;
            isAutoPlay = false;
        }
        ...
        if (GameCanvas.keyPressed[!Main.isPC ? 5 : 25] && !isPaintPopup())
        {
            if (auto == 0)
            {
                if (num - lastFire < 800 && checkSkillValid2() && (Char.myCharz().mobFocus != null ||
                                                                   (Char.myCharz().charFocus != null &&
                                                                    Char.myCharz()
                                                                        .isMeCanAttackOtherPlayer(Char.myCharz()
                                                                            .charFocus))))
                {
                    Res.outz("toi day");
                    auto = 10;
        ...
        {
            if (Char.myCharz().myskill != null && (Char.myCharz().myskill.template.isUseAlone() ||
                                                   Char.myCharz().myskill.paintCanNotUseSkill))
                return;
            if ((Char.myCharz().mobFocus != null && Char.myCharz().mobFocus.status != 1 &&
                 Char.myCharz().mobFocus.status != 0 && Char.myCharz().charFocus == null) ||
                (Char.myCharz().charFocus != null && Char.myCharz().isMeCanAttackOtherPlayer(Char.myCharz().charFocus)))
            {
    ...
    internal void autoPlay()
    {
        if (timeSkill > 0)
            timeSkill--;
        if (!canAutoPlay || isChangeZone || Char.myCharz().statusMe == 14 || Char.myCharz().statusMe == 5 ||
            Char.myCharz().isCharge || Char.myCharz().isFlyAndCharge || Char.myCharz().isUseChargeSkill())
            return;
        var flag = false;
        for (var i = 0; i < vMob.size(); i++)
        {
            var mob = (Mob)vMob.elementAt(i);
        ...
        if (Char.myCharz().cHP <= Char.myCharz().cHPFull * 20 / 100 ||
            Char.myCharz().cMP <= Char.myCharz().cMPFull * 20 / 100)
            doUseHP();
        if (Char.myCharz().mobFocus == null || (Char.myCharz().mobFocus != null && Char.myCharz().mobFocus.isMobMe))
            for (var k = 0; k < vMob.size(); k++)
            {
                var mob2 = (Mob)vMob.elementAt(k);
        ...
        else if (Char.myCharz().mobFocus.hp <= 0 || Char.myCharz().mobFocus.status == 1 ||
                 Char.myCharz().mobFocus.status == 0) Char.myCharz().mobFocus = null;

        if (Char.myCharz().mobFocus == null || timeSkill != 0 || (Char.myCharz().skillInfoPaint() != null &&
                                                                  Char.myCharz().indexSkill <
                                                                  Char.myCharz().skillInfoPaint().Length &&
                                                                  Char.myCharz().dart != null &&
                                                                  Char.myCharz().arr != null))
            return;
        Skill skill = null;
        if (GameCanvas.isTouch)
        {
            for (var l = 0; l < onScreenSkill.Length; l++)
            {
                if (onScreenSkill[l] == null || onScreenSkill[l].paintCanNotUseSkill ||
                    onScreenSkill[l].template.id == 10 || onScreenSkill[l].template.id == 11 ||
                    onScreenSkill[l].template.id == 14 || onScreenSkill[l].template.id == 23 ||
                    onScreenSkill[l].template.id == 7 || Char.myCharz().skillInfoPaint() != null ||
                    onScreenSkill[l].template.isSkillSpec())
                    continue;
                var num = 0;
                num = onScreenSkill[l].template.manaUseType == 2
                    ? 1
                    :
                    onScreenSkill[l].template.manaUseType == 1
                        ?
                        onScreenSkill[l].manaUse * Char.myCharz().cMPFull / 100
                        : onScreenSkill[l].manaUse;
                if (Char.myCharz().cMP >= num)
                {
                    if (skill == null)
                        skill = onScreenSkill[l];
                    else if (skill.coolDown < onScreenSkill[l].coolDown) skill = onScreenSkill[l];
                }
            }
        ...
        for (var m = 0; m < keySkill.Length; m++)
        {
            if (keySkill[m] == null || keySkill[m].paintCanNotUseSkill || keySkill[m].template.id == 10 ||
                keySkill[m].template.id == 11 || keySkill[m].template.id == 14 || keySkill[m].template.id == 23 ||
                keySkill[m].template.id == 7 || Char.myCharz().skillInfoPaint() != null)
                continue;
            var num2 = 0;
            num2 = keySkill[m].template.manaUseType == 2 ? 1 :
                keySkill[m].template.manaUseType == 1 ? keySkill[m].manaUse * Char.myCharz().cMPFull / 100 :
                keySkill[m].manaUse;
            if (Char.myCharz().cMP >= num2)
            {
                if (skill == null)
                    skill = keySkill[m];
                else if (skill.coolDown < keySkill[m].coolDown) skill = keySkill[m];
            }
        }
    ...
    {
        tam++;
        var waypoint = Char.myCharz().isInEnterOfflinePoint();
        var waypoint2 = Char.myCharz().isInEnterOnlinePoint();
        if (!skipWaypoint && waypoint != null && (Char.myCharz().mobFocus == null ||
                                                  (Char.myCharz().mobFocus != null &&
                                                   Char.myCharz().mobFocus.templateId == 0)))
        {
            waypoint.popup.command.performAction();
        }
        else if (!skipWaypoint && waypoint2 != null && (Char.myCharz().mobFocus == null ||
                                                        (Char.myCharz().mobFocus != null &&
                                                         Char.myCharz().mobFocus.templateId == 0)))
        {
        ...
        else
        {
            ...
            else if (isAttack())
            {
                ...
                {
                    var flag = TileMap.tileTypeAt(Char.myCharz().cx, Char.myCharz().cy, 2);
    ...
    {
        var npc = new Npc(5, 0, -100, 100, 5, info1.charId[Char.myCharz().cgender][2]);
        var nhatvatpham = mResources.nhatvatpham;
        var menu = new string[2]
        {
            mResources.YES,
            mResources.NO
        };
    ...
    internal void pickItem()
    {
        ...
        if (Char.myCharz().cx < Char.myCharz().itemFocus.x)
            ...
            Char.myCharz().cdir = -1;
        var num = Math2.abs(Char.myCharz().cx - Char.myCharz().itemFocus.x);
        var num2 = Math2.abs(Char.myCharz().cy - Char.myCharz().itemFocus.y);
        if (num <= 40 && num2 < 40)
        {
            GameCanvas.clearKeyHold();
            GameCanvas.clearKeyPressed();
    ...
    {
        if (Char.myCharz().isFlyAndCharge || Char.myCharz().isUseSkillAfterCharge || Char.myCharz().isStandAndCharge ||
            Char.myCharz().isWaitMonkey || isSuperPower || Char.myCharz().isFreez)
            return true;
        return false;
    }
    ...
    {
        for (var i = 0; i < Char.myCharz().vSkillFight.size() - 1; i++)
        {
            var skill = (Skill)Char.myCharz().vSkillFight.elementAt(i);
            for (var j = i + 1; j < Char.myCharz().vSkillFight.size(); j++)
            {
                var skill2 = (Skill)Char.myCharz().vSkillFight.elementAt(j);
    ...
    public void updateKeyTouchCapcha()
    {
        if (isNotPaintTouchControl())
            return;
        for (var i = 0; i < strCapcha.Length; i++)
        {
            keyCapcha[i] = -1;
            if (!GameCanvas.isTouchControl)
                continue;
            var num = (GameCanvas.w - strCapcha.Length * disXC) / 2;
            var w = strCapcha.Length * disXC;
            if (!GameCanvas.isPointerHoldIn(num, GameCanvas.h - 40, w, disXC))
                continue;
            var num2 = (GameCanvas.px - num) / disXC;
            if (i == num2)
                keyCapcha[i] = 1;
    ...
    internal void updateKeyTouchControl()
    {
        ...
        if (GameCanvas.isTouchControl)
        {
            if (GameCanvas.isPointerHoldIn(0, 0, 60, 50) && GameCanvas.isPointerClick &&
            ...
            if (Char.myCharz().cmdMenu != null && GameCanvas.isPointerHoldIn(Char.myCharz().cmdMenu.x - 17,
                    Char.myCharz().cmdMenu.y - 17, 34, 34))
            {
                keyTouch = 20;
                GameCanvas.isPointerJustDown = false;
                isPointerDowning = false;
            ...
            updateGamePad();
            if ((isAnalog != 0
                    ? GameCanvas.isPointerHoldIn(xHP, yHP + 10, 34, 34)
                    : GameCanvas.isPointerHoldIn(xHP, yHP + 10, 40, 40)) && Char.myCharz().statusMe != 14 &&
                mobCapcha == null)
            {
                keyTouch = 10;
                GameCanvas.isPointerJustDown = false;
            ...
            if ((isAnalog != 0
                    ? GameCanvas.isPointerHoldIn(xHP + 5, yHP - 6 - 34 + 10, 34, 34)
                    : GameCanvas.isPointerHoldIn(xHP + 5, yHP - 6 - 40 + 10, 40, 40)) &&
                Char.myCharz().statusMe != 14 && mobCapcha == null)
            {
                if (isPickNgocRong)
                {
                    ...
                    if (GameCanvas.isPointerClick && GameCanvas.isPointerJustRelease)
                    {
                        ...
                        GameCanvas.isPointerClick =
                            GameCanvas.isPointerJustDown = GameCanvas.isPointerJustRelease = false;
                ...
                else if (isudungCapsun3)
                {
                    ...
                    if (GameCanvas.isPointerClick && GameCanvas.isPointerJustRelease)
                    {
                        ...
                        for (var j = 0; j < Char.myCharz().arrItemBag.Length; j++)
                        {
                            var item2 = Char.myCharz().arrItemBag[j];
        ...
        else if (isHaveSelectSkill)
        {
            ...
            keyTouchSkill = -1;
            var flag = false;
            if (onScreenSkill.Length > 5 &&
                (GameCanvas.isPointerHoldIn(xSkill + xS[0] - wSkill / 2 + 12, yS[0] - wSkill / 2 + 12, 5 * wSkill,
                    wSkill) || GameCanvas.isPointerHoldIn(xSkill + xS[5] - wSkill / 2 + 12, yS[5] - wSkill / 2 + 12,
                    5 * wSkill, wSkill)))
                flag = true;
            if (flag ||
                GameCanvas.isPointerHoldIn(xSkill + xS[0] - wSkill / 2 + 12, yS[0] - wSkill / 2 + 12, 5 * wSkill,
                    wSkill) || (!GameCanvas.isTouchControl && GameCanvas.isPointerHoldIn(
                    xSkill + xS[0] - wSkill / 2 + 12, yS[0] - wSkill / 2 + 12, wSkill, onScreenSkill.Length * wSkill)))
            {
                GameCanvas.isPointerJustDown = false;
                isPointerDowning = false;
                var num = (GameCanvas.pxLast - (xSkill + xS[0] - wSkill / 2 + 12)) / wSkill;
        ...
        if (GameCanvas.isPointerJustRelease)
        {
            if (GameCanvas.keyHold[1] || GameCanvas.keyHold[!Main.isPC ? 2 : 21] || GameCanvas.keyHold[3] ||
                GameCanvas.keyHold[!Main.isPC ? 4 : 23] || GameCanvas.keyHold[!Main.isPC ? 6 : 24])
                GameCanvas.isPointerJustRelease = false;
    ...
    public void updateXoSo()
    {
        ...
        if (secondXS > 20)
        {
            for (var i = 0; i < winnumber.Length; i++) randomNumber[i] = winnumber[i];
            tShow--;
            if (tShow == 0)
            {
        ...
        for (var j = 0; j < winnumber.Length; j++)
        {
            if (tMove[j] == -1)
                continue;
            moveCount[j]++;
    ...
    public override void update()
    {
        ...
        try
        {
            ...
            if (tickMove == 100)
            {
                ...
                var second = (int)(currTick - lastTick) / 1000;
            ...
            for (var i = 0; i < vCharInMap.size(); i++) ((Char)vCharInMap.elementAt(i)).update();
            for (var i = 0; i < Teleport.vTeleport.size(); i++) ((Teleport)Teleport.vTeleport.elementAt(i)).update();
            Char.myCharz().update();
            if (Char.myCharz().statusMe == 1)
                ;
            if (popUpYesNo != null)
            ...
            for (var i = 0; i < vMob.size(); i++) ((Mob)vMob.elementAt(i)).update();
            GameCanvas.debug("E6", 0);
            for (var i = 0; i < vNpc.size(); i++) ((Npc)vNpc.elementAt(i)).update();
            nSkill = onScreenSkill.Length;
            for (var i = onScreenSkill.Length - 1; i >= 0; i--)
            {
                if (onScreenSkill[i] != null)
                {
                    nSkill = i + 1;
            ...
            for (var i = 0; i < vItemMap.size(); i++) ((ItemMap)vItemMap.elementAt(i)).update();
            GameCanvas.debug("E11", 0);
            GameCanvas.debug("E13", 0);
            for (var i = Effect2.vRemoveEffect2.size() - 1; i >= 0; i--)
            {
                Effect2.vEffect2.removeElement(Effect2.vRemoveEffect2.elementAt(i));
                Effect2.vRemoveEffect2.removeElementAt(i);
            }

            for (var i = 0; i < Effect2.vEffect2.size(); i++) ((Effect2)Effect2.vEffect2.elementAt(i)).update();
            for (var i = 0; i < Effect2.vEffect2Outside.size(); i++)
                ((Effect2)Effect2.vEffect2Outside.elementAt(i)).update();
            for (var i = 0; i < Effect2.vAnimateEffect.size(); i++)
                ((Effect2)Effect2.vAnimateEffect.elementAt(i)).update();
            for (var i = 0; i < Effect2.vEffectFeet.size(); i++) ((Effect2)Effect2.vEffectFeet.elementAt(i)).update();
            for (var i = 0; i < Effect2.vEffect3.size(); i++) ((Effect2)Effect2.vEffect3.elementAt(i)).update();
            ...
            {
                var num = (dHP - Char.myCharz().cHP) >> 1;
                if (num < 1)
                    num = 1;
                dHP -= num;
            }
            ...
            {
                var num2 = (dMP - Char.myCharz().cMP) >> 1;
                if (num2 < 1)
                    num2 = 1;
                dMP -= num2;
            }
            ...
            {
                var num3 = 100;
                while (yR - num3 < cmy) cmy--;
            }

            for (var i = 0; i < Char.vItemTime.size(); i++) ((ItemTime)Char.vItemTime.elementAt(i)).update();
            for (var i = 0; i < textTime.size(); i++) ((ItemTime)textTime.elementAt(i)).update();
            updateChatVip();
        }
        catch (Exception)
        {
    ...
    {
        for (var i = 0; i < Effect2.vEffect2.size(); i++)
        {
            var effect = (Effect2)Effect2.vEffect2.elementAt(i);
    ...
    public void paintCapcha(mGraphics g)
    {
        ...
        if (GameCanvas.menu.showMenu || GameCanvas.panel.isShow || ChatPopup.currChatPopup != null ||
            !GameCanvas.isTouch)
            return;
        for (var i = 0; i < strCapcha.Length; i++)
        {
            var x = (GameCanvas.w - strCapcha.Length * disXC) / 2 + i * disXC + disXC / 2;
            if (keyCapcha[i] == -1)
            {
    ...
    public override void paint(mGraphics g)
    {
        ...
        {
            dem++;
            if ((dem < 30 && dem >= 0 && GameCanvas.gameTick % 4 == 0) ||
                (dem >= 30 && dem <= 50 && GameCanvas.gameTick % 3 == 0) || dem > 50)
            {
                g.setColor(16777215);
        ...
        {
            var @char = (Char)vCharInMap.elementAt(i);
        ...
        {
            if (keyTouch == 20)
                g.drawImage(imgChat2, Char.myCharz().cmdMenu.x + cmx, Char.myCharz().cmdMenu.y + cmy,
                    mGraphics.HCENTER | mGraphics.VCENTER);
            else
                g.drawImage(imgChat, Char.myCharz().cmdMenu.x + cmx, Char.myCharz().cmdMenu.y + cmy,
                    mGraphics.HCENTER | mGraphics.VCENTER);
        }
        ...
        for (var i = 0; i < Effect2.vEffectFeet.size(); i++) ((Effect2)Effect2.vEffectFeet.elementAt(i)).paint(g);
        for (var i = 0; i < Teleport.vTeleport.size(); i++) ((Teleport)Teleport.vTeleport.elementAt(i)).paintHole(g);
        for (var i = 0; i < vNpc.size(); i++)
        {
            var npc = (Npc)vNpc.elementAt(i);
            if (npc.cHP > 0)
                npc.paintShadow(g);
        }

        for (var i = 0; i < vNpc.size(); i++) ((Npc)vNpc.elementAt(i)).paint(g);
        g.translate(0, GameCanvas.transY);
        ...
        for (var i = 0; i < vMob.size(); i++) ((Mob)vMob.elementAt(i)).paint(g);
        for (var i = 0; i < Teleport.vTeleport.size(); i++) ((Teleport)Teleport.vTeleport.elementAt(i)).paint(g);
        for (var i = 0; i < vCharInMap.size(); i++)
        {
            Char char3 = null;
            try
        ...
        Char.myCharz().paint(g);
        if (Char.myCharz().skillPaint != null && Char.myCharz().skillInfoPaint() != null &&
            Char.myCharz().indexSkill < Char.myCharz().skillInfoPaint().Length)
        {
        ...
        for (var i = 0; i < vCharInMap.size(); i++)
        {
            ...
            if (char4 != null && (!GameCanvas.panel.isShow || !GameCanvas.panel.isTypeShop()) &&
                char4.skillPaint != null && char4.skillInfoPaint() != null &&
                char4.indexSkill < char4.skillInfoPaint().Length)
            {
        ...
        for (var i = 0; i < vItemMap.size(); i++) ((ItemMap)vItemMap.elementAt(i)).paint(g);
        g.translate(0, -GameCanvas.transY);
        ...
        paintEffect(g);
        paintBgItem(g, 3);
        for (var i = 0; i < vNpc.size(); i++) ((Npc)vNpc.elementAt(i)).paintName(g);
        EffecMn.paintLayer3(g);
        for (var i = 0; i < vNpc.size(); i++)
        {
            var npc2 = (Npc)vNpc.elementAt(i);
        ...
        if (Char.myCharz().chatInfo != null)
            Char.myCharz().chatInfo.paint(g, Char.myCharz().cx, Char.myCharz().cy - Char.myCharz().ch,
                Char.myCharz().cdir);
        EffectManager.mid_2Effects.paintAll(g);
        EffectManager.midEffects.paintAll(g);
        BackgroudEffect.paintFrontAll(g);
        for (var j = 0; j < TileMap.vCurrItem.size(); j++)
        {
            var bgItem = (BgItem)TileMap.vCurrItem.elementAt(j);
        ...
        if (TileMap.mapID == 120)
        {
            if (percentMabu != 100)
            {
                var w = percentMabu * mGraphics.getImageWidth(imgHPLost) / 100;
                int num = percentMabu;
            ...
            if (mabuEff)
            {
                ...
                if (GameCanvas.gameTick % 3 == 0)
                    EffecMn.addEff(
                        new Effect(19, Res.random(TileMap.pxw / 2 - 50, TileMap.pxw / 2 + 50), 340, 2, 1, -1));
                if (GameCanvas.gameTick % 15 == 0)
                    EffecMn.addEff(new Effect(18, Res.random(TileMap.pxw / 2 - 5, TileMap.pxw / 2 + 5),
                        Res.random(300, 320), 2, 1, -1));
                if (tMabuEff == 100)
        ...
        var flag = true;
        for (var i = 0; i < BackgroudEffect.vBgEffect.size(); i++)
        ...
        {
            var num2 = TileMap.pxw / (mGraphics.getImageWidth(TileMap.imgLight) + 50);
            if (num2 <= 0)
                num2 = 1;
            if (TileMap.tileID != 28)
                for (var i = 0; i < num2; i++)
                {
                    var num3 = 100 + i * (mGraphics.getImageWidth(TileMap.imgLight) + 50) - cmx / 2;
                    var num4 = -20;
                    if (num3 + mGraphics.getImageWidth(TileMap.imgLight) >= cmx && num3 <= cmx + GameCanvas.w &&
                        num4 + mGraphics.getImageHeight(TileMap.imgLight) >= cmy && num4 <= cmy + GameCanvas.h)
                        g.drawImage(TileMap.imgLight,
                            100 + i * (mGraphics.getImageWidth(TileMap.imgLight) + 50) - cmx / 2, num4, 0);
                }
        }
        ...
        if (!isPaintOther)
        {
            if (GameCanvas.open3Hour && TileMap.mapID != 170)
            {
                ...
                else
                {
                    ...
                    mFont.tahoma_7_white.drawString(g, "Chơi quá 180 phút mỗi ngày sẽ hại sức khỏe.", 25,
                        GameCanvas.h - 60, 0);
                }
            }
            ...
            if (!GameCanvas.panel.isShow && GameCanvas.currentDialog == null && ChatPopup.currChatPopup == null &&
                ChatPopup.serverChatPopUp == null && GameCanvas.currentScreen.Equals(instance))
            {
                base.paint(g);
            ...
            resetTranslate(g);
            var num7 = 100 + (Char.vItemTime.size() != 0 ? textTime.size() * 12 : 0);
            if (Char.myCharz().clan != null)
            {
                var num8 = 0;
                var num9 = 0;
                var num10 = (GameCanvas.h - 100 - 60) / 12;
                for (var i = 0; i < vCharInMap.size(); i++)
                {
                    var char7 = (Char)vCharInMap.elementAt(i);
                    ...
                    if (char7.isOutX() && char7.cx < Char.myCharz().cx)
                    {
                        ...
                        if (num8 <= num11)
                        {
                            mFont.tahoma_7_green.drawString(g, char7.cName, 20, num7 - 12 + num8 * 12, mFont.LEFT,
                                mFont.tahoma_7_grey);
                    ...
                    {
                        mFont.tahoma_7_green.drawString(g, char7.cName, GameCanvas.w - 25, num7 - 12 + num9 * 12,
                            mFont.RIGHT, mFont.tahoma_7_grey);
            ...
            for (var i = 0; i < Char.vItemTime.size(); i++)
                ((ItemTime)Char.vItemTime.elementAt(i)).paint(g, cmdMenu.x + 32 + i * 24, 55);
            for (var i = 0; i < textTime.size(); i++)
                ((ItemTime)textTime.elementAt(i)).paintText(g, cmdMenu.x + (Char.vItemTime.size() == 0 ? 25 : 5),
                    (Char.vItemTime.size() == 0 ? 45 : 90) + i * 12);
            paintXoSo(g);
            if (mResources.language == 1)
            {
                var second = mSystem.currentTimeMillis() - deltaTime;
                mFont.tahoma_7b_white.drawString(g, NinjaUtil.getDate2(second), 10, GameCanvas.h - 65, 0,
                    mFont.tahoma_7b_dark);
            }

            if (!yourNumber.Equals(string.Empty))
                for (var i = 0; i < strPaint.Length; i++)
                    mFont.tahoma_7b_white.drawString(g, strPaint[i], 5, 85 + i * 18, 0, mFont.tahoma_7b_dark);
        }

        var num12 = 0;
        var num13 = GameCanvas.hw;
        if (num13 > 200)
            num13 = 200;
        paintPhuBanBar(g, num12 + GameCanvas.w / 2, 0, num13);
        EffectManager.hiEffects.paintAll(g);
        if (nCT_timeBallte > mSystem.currentTimeMillis() && TileMap.mapID == 170 && isPaint_CT &&
            nCT_nBoyBaller / 2 > 0)
            try
            {
        ...
        {
            var text = mResources.WAIT + "  " + nUSER_CT + "/" + nUSER_MAX_CT;
            mFont.tahoma_7b_dark.drawString(g, mResources.WAIT + "  " + nUSER_CT + "/" + nUSER_MAX_CT,
    ...
    internal void paintXoSo(mGraphics g)
    {
        if (tShow != 0)
        {
            ...
            for (var i = 0; i < winnumber.Length; i++) text = text + randomNumber[i] + " ";
    ...
    internal void checkEffToObj(IMapObject obj, bool isnew)
    {
        ...
        var x = obj.getX();
        var num = 1;
        var num2 = Res.abs(Char.myCharz().cx - x);
        num = num2 <= 80 ? 1 : num2 > 80 && num2 <= 200 ? 2 : num2 <= 200 || num2 > 400 ? 4 : 3;
        if (!isnew)
        {
            if (obj.Equals(Char.myCharz().mobFocus) || (obj.Equals(Char.myCharz().charFocus) &&
                                                        Char.myCharz()
                                                            .isMeCanAttackOtherPlayer(Char.myCharz().charFocus)))
                ServerEffect.addServerEffect(135, obj.getX(), obj.getY(), num);
            else if (obj.Equals(Char.myCharz().npcFocus) || obj.Equals(Char.myCharz().itemFocus) ||
                     obj.Equals(Char.myCharz().charFocus))
    ...
    internal void updateClickToArrow()
    {
        ...
        {
            clickMoving = false;
            var mapObject = findClickToItem(clickToX, clickToY);
            if (mapObject == null ||
                (mapObject != null && mapObject.Equals(Char.myCharz().npcFocus) && TileMap.mapID == 51))
    ...
    {
        var num = 10;
        var taskMaint = Char.myCharz().taskMaint;
        if (taskMaint != null && taskMaint.taskId == 0 &&
            ((taskMaint.index != 1 && taskMaint.index < 6) || taskMaint.index == 0))
            return;
        for (var i = 0; i < TileMap.vGo.size(); i++)
        {
            var waypoint = (Waypoint)TileMap.vGo.elementAt(i);
            if (waypoint.minY == 0 || waypoint.maxY >= TileMap.pxh - 24)
            {
                if (waypoint.maxY <= TileMap.pxh / 2)
                {
                    var x = waypoint.minX + (waypoint.maxX - waypoint.minX) / 2;
                    var y = waypoint.minY + (waypoint.maxY - waypoint.minY) / 2 + runArrow;
                    if (GameCanvas.isTouch)
                ...
                {
                    g.drawRegion(arrow, 0, 0, 13, 16, 4, waypoint.minX + (waypoint.maxX - waypoint.minX) / 2,
                        waypoint.minY - 12 - runArrow, StaticObj.VCENTER_HCENTER);
                }
            }
            else if (waypoint.minX >= 0 && waypoint.minX < 24)
            {
                if (!GameCanvas.isTouch)
                    g.drawRegion(arrow, 0, 0, 13, 16, 2, waypoint.maxX + 12 + runArrow, waypoint.maxY - 12,
                        StaticObj.VCENTER_HCENTER);
                else
                    g.drawRegion(arrow, 0, 0, 13, 16, 2, waypoint.maxX + 12 + runArrow, waypoint.maxY - 32,
                        StaticObj.VCENTER_HCENTER);
            }
            else if (waypoint.minX <= TileMap.tmw * 24 && waypoint.minX >= TileMap.tmw * 24 - 48)
            {
                if (!GameCanvas.isTouch)
                    g.drawRegion(arrow, 0, 0, 13, 16, 0, waypoint.minX - 12 - runArrow, waypoint.maxY - 12,
                        StaticObj.VCENTER_HCENTER);
                else
                    g.drawRegion(arrow, 0, 0, 13, 16, 0, waypoint.minX - 12 - runArrow, waypoint.maxY - 32,
                        StaticObj.VCENTER_HCENTER);
            }
            else
            {
                g.drawRegion(arrow, 0, 0, 13, 16, 4, waypoint.minX + (waypoint.maxX - waypoint.minX) / 2,
                    waypoint.maxY - 48 - runArrow, StaticObj.VCENTER_HCENTER);
            }
        }
    }
        ...
        try
        {
            ...
            for (var i = 0; i < vNpc.size(); i++)
            {
                ...
                if (npc2.template.npcTemplateId == num)
                {
                    if (npc == null)
                        npc = npc2;
                    else if (Res.abs(npc2.cx - Char.myCharz().cx) < Res.abs(npc.cx - Char.myCharz().cx)) npc = npc2;
                }
            }

            if (npc == null || npc.statusMe == 15 ||
                (npc.cx > cmx && npc.cx < cmx + gW && npc.cy > cmy && npc.cy < cmy + gH) ||
                GameCanvas.gameTick % 10 < 5)
                return;
            var num2 = npc.cx - Char.myCharz().cx;
            var num3 = npc.cy - Char.myCharz().cy;
            var x = 0;
            var y = 0;
            var arg = 0;
            if (num2 > 0 && num3 >= 0)
    ...
    internal void paintTouchControl(mGraphics g)
    {
        ...
        if (!TileMap.isOfflineMap() && !isVS())
        {
            if (keyTouch == 15 || keyMouse == 15)
                g.drawImage(!Main.isPC ? imgChat2 : imgChatsPC2, xC + 17, yC + 17 + mGraphics.addYWhenOpenKeyBoard,
                    mGraphics.HCENTER | mGraphics.VCENTER);
            else
                g.drawImage(!Main.isPC ? imgChat : imgChatPC, xC + 17, yC + 17 + mGraphics.addYWhenOpenKeyBoard,
                    mGraphics.HCENTER | mGraphics.VCENTER);
        }
    ...
    {
        var num = (int)(c.cHP * hpBarW / c.cHPFull);
        var num2 = c.cMP * mpBarW;
        var num3 = (int)(dHP * hpBarW / c.cHPFull);
        var num4 = dMP * mpBarW;
        g.setClip(GameCanvas.w / 2 + 58 - mGraphics.getImageWidth(imgPanel), 0, 95, 100);
        g.drawRegion(imgPanel, 0, 0, mGraphics.getImageWidth(imgPanel), mGraphics.getImageHeight(imgPanel), 2,
            GameCanvas.w / 2 + 60, 0, mGraphics.RIGHT | mGraphics.TOP);
    ...
    internal void paintImageBar(mGraphics g, bool isLeft, Char c)
    {
        if (c != null)
        {
            ...
            {
                var w = Char.myCharz().powerPoint * spBarW / Char.myCharz().maxPowerPoint;
                g.drawImage(imgPanel2, 58, 29, 0);
                g.setClip(83, 31, w, 10);
                g.drawImage(imgSP, 83, 31, 0);
                g.setClip(0, 0, GameCanvas.w, GameCanvas.h);
                mFont.tahoma_7_white.drawString(g,
                    Char.myCharz().strInfo + ":" + Char.myCharz().powerPoint + "/" + Char.myCharz().maxPowerPoint, 115,
                    29, 2);
            }
    ...
    internal void paintInfoBar(mGraphics g)
    {
        ...
        else
        {
            ...
            {
                mFont.tahoma_7_green2.drawString(g, mResources.enter, imgScrW / 2, 8 + mGraphics.addYWhenOpenKeyBoard,
                    mFont.CENTER);
            }
            else if (Char.myCharz().mobFocus != null)
            {
                if (Char.myCharz().mobFocus.getTemplate() != null)
                    mFont.tahoma_7b_green2.drawString(g, Char.myCharz().mobFocus.getTemplate().name, imgScrW / 2,
                        9 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
                if (Char.myCharz().mobFocus.templateId != 0)
                    mFont.tahoma_7b_green2.drawString(g, NinjaUtil.getMoneys(Char.myCharz().mobFocus.hp) + string.Empty,
                        imgScrW / 2, 22 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
            }
            else if (Char.myCharz().npcFocus != null)
            {
                mFont.tahoma_7b_green2.drawString(g, Char.myCharz().npcFocus.template.name, imgScrW / 2,
                    9 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
                if (Char.myCharz().npcFocus.template.npcTemplateId == 4)
                    mFont.tahoma_7b_green2.drawString(g, gI().magicTree.currPeas + "/" + gI().magicTree.maxPeas,
                        imgScrW / 2, 22 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
            }
            else if (Char.myCharz().charFocus != null)
            {
                mFont.tahoma_7b_green2.drawString(g, Char.myCharz().charFocus.cName, imgScrW / 2,
                    9 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
                mFont.tahoma_7b_green2.drawString(g, NinjaUtil.getMoneys(Char.myCharz().charFocus.cHP) + string.Empty,
                    imgScrW / 2, 22 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
            }
            else
            {
                mFont.tahoma_7b_green2.drawString(g, Char.myCharz().cName, imgScrW / 2,
                    9 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
                mFont.tahoma_7b_green2.drawString(g, NinjaUtil.getMoneys(Char.myCharz().cPower) + string.Empty,
                    imgScrW / 2, 22 + mGraphics.addYWhenOpenKeyBoard, mFont.CENTER);
            }
        }
    ...
    public bool isVS()
    {
        if (TileMap.isVoDaiMap() && (Char.myCharz().cTypePk != 0 ||
                                     (TileMap.mapID == 130 && findCharVS1() != null && findCharVS2() != null)))
            return true;
        return false;
    }
    ...
    internal void paintSelectedSkill(mGraphics g)
    {
        ...
        {
            if (GameCanvas.currentDialog != null || ChatPopup.currChatPopup != null || GameCanvas.menu.showMenu ||
                isPaintPopup() || GameCanvas.panel.isShow || Char.myCharz().taskMaint.taskId == 0 ||
                ChatTextField.gI().isShow || GameCanvas.currentScreen == MoneyCharge.instance)
                return;
            var num = mSystem.currentTimeMillis() - lastUsePotion;
            var num2 = 0;
            if (num < 10000)
            ...
            else if (Char.myCharz().statusMe != 14)
            {
                if (gamePad.isSmallGamePad)
                {
                    ...
                    {
                        var num3 = 10;
                        g.drawImage(keyTouch != 10 ? imgSkill : imgSkill2, xSkill + xHP - 1, yHP - 1 + num3, 0);
                        ...
                        if (isPickNgocRong)
                        {
                            g.drawImage(keyTouch != 14 ? imgNR3 : imgNR4, xHP + 20 + 5, yHP + 20 - 6 - 40 + 10,
                                mGraphics.HCENTER | mGraphics.VCENTER);
                        }
                        else if (isudungCapsun4)
                        {
                            g.drawImage(keyTouch != 14 ? imgNut : imgNutF, xHP + 20 + 5, yHP + 20 - 6 - 40 + 10,
                                mGraphics.HCENTER | mGraphics.VCENTER);
                ...
                else if (isAnalog != 1)
                {
                    ...
                    {
                        g.drawImage(keyTouch != 14 ? imgNut : imgNutF, xHP + 20, yHP + 20 - 6 - 40,
                            mGraphics.HCENTER | mGraphics.VCENTER);
                ...
                else
                {
                    ...
                    g.drawImage(keyTouch != 10 ? imgHP3 : imgHP4, xHP + 20, yHP + 20 - 6 + 10,
                        mGraphics.HCENTER | mGraphics.VCENTER);
            ...
            {
                var array = Main.isPC ? keySkill : !GameCanvas.isTouch ? keySkill : onScreenSkill;
                if (keyTouch == 10)
                    ;
                if (!GameCanvas.isTouch)
                {
                ...
                var num4 = Main.isPC ? array.Length : !GameCanvas.isTouch ? array.Length : nSkill;
                for (var i = 0; i < num4; i++)
                {
                    if (Main.isPC)
                    {
                        ...
                        mFont.tahoma_7b_white.drawString(g, array2[i], xSkill + xS[i] + 14, yS[i] + num5 + 1,
                            mFont.CENTER);
                    }
                    else if (!GameCanvas.isTouch)
                    {
                        var array3 = TField.isQwerty
                            ? new string[5] { "Q", "W", "E", "R", "T" }
                            : new string[5] { "7", "8", "9", "1", "3" };
                        g.setColor(11152401);
                        g.fillRect(xSkill + xS[i] + 2, yS[i] - 10 + 8, 20, 10);
                        mFont.tahoma_7_white.drawString(g, array3[i], xSkill + xS[i] + 12, yS[i] - 10 + 6,
                            mFont.CENTER);
                    }
    ...
    public static void startFlyText(string flyString, int x, int y, int dx, int dy, int color)
    {
        ...
        flyTextYTo[num] = 10;
        for (var j = 0; j < 5; j++)
            if (flyTextState[j] != -1 && num != j && flyTextDy[num] < 0 && Res.abs(flyTextX[num] - flyTextX[j]) <= 20 &&
                flyTextYTo[num] == flyTextYTo[j])
                flyTextYTo[num] += 10;
    }
    ...
    public static void loadSplash()
    {
        if (imgSplash == null)
        {
            ...
            for (var i = 0; i < 3; i++) imgSplash[i] = GameCanvas.loadImage("/e/sp" + i + ".png");
        }
        ...
        for (var i = 0; i < 2; i++)
            if (splashState[i] != -1)
            {
                if (splashDir[i] == 1)
                    ...
                    g.drawRegion(imgSplash[splashF[i]], 0, 0, mGraphics.getImageWidth(imgSplash[splashF[i]]),
                        mGraphics.getImageHeight(imgSplash[splashF[i]]), 2, splashX[i], splashY[i], 3);
            }
    }
    ...
    public void updateKeyAlert()
    {
        ...
        if (GameCanvas.isTouch)
        {
            var scrollResult = scrMain.updateKey();
        ...
        if (!flag || indexRow < 0 || indexRow >= texts.size())
            return;
        var text = (string)texts.elementAt(indexRow);
        var num = -1;
        fnick = null;
        alertURL = null;
        center = null;
        ...
        else
        {
            if ((num = text.IndexOf("@")) < 0)
                return;
            var text2 = text.Substring(2).Trim();
            num = text2.IndexOf("@");
            var text3 = text2.Substring(num);
            var num2 = -1;
            num2 = text3.IndexOf(" ");
    ...
    {
        if (isPaintItemInfo || isPaintInfoMe || isPaintStore || isPaintWeapon || isPaintNonNam || isPaintNonNu ||
            isPaintAoNam || isPaintAoNu || isPaintGangTayNam || isPaintGangTayNu || isPaintQuanNam || isPaintQuanNu ||
            isPaintGiayNam || isPaintGiayNu || isPaintLien || isPaintNhan || isPaintNgocBoi || isPaintPhu ||
            isPaintStack || isPaintStackLock || isPaintGrocery || isPaintGroceryLock || isPaintUpGrade ||
            isPaintConvert || isPaintSplit || isPaintUpPearl || isPaintBox || isPaintTrade || isPaintAlert ||
            isPaintZone || isPaintTeam || isPaintClan || isPaintFindTeam || isPaintTask || isPaintFriend ||
            isPaintEnemies || isPaintCharInMap || isPaintMessage)
            return true;
        return false;
    }
    ...
    public bool isNotPaintTouchControl()
    {
        ...
        if (InfoDlg.isShow)
            return true;
        if (GameCanvas.currentDialog != null || ChatPopup.currChatPopup != null || GameCanvas.menu.showMenu ||
            GameCanvas.panel.isShow || isPaintPopup())
            return true;
        return false;
    }
    ...
    {
        if (isPaintStore || isPaintWeapon || isPaintNonNam || isPaintNonNu || isPaintAoNam || isPaintAoNu ||
            isPaintGangTayNam || isPaintGangTayNu || isPaintQuanNam || isPaintQuanNu || isPaintGiayNam ||
            isPaintGiayNu || isPaintLien || isPaintNhan || isPaintNgocBoi || isPaintPhu || isPaintStack ||
            isPaintStackLock || isPaintGrocery || isPaintGroceryLock || isPaintUpGrade || isPaintConvert ||
            isPaintSplit || isPaintUpPearl || isPaintBox || isPaintTrade)
            return true;
        return false;
    }
    ...
    {
        if (isPaintItemInfo || isPaintInfoMe || isPaintStore || isPaintNonNam || isPaintNonNu || isPaintAoNam ||
            isPaintAoNu || isPaintGangTayNam || isPaintGangTayNu || isPaintQuanNam || isPaintQuanNu || isPaintGiayNam ||
            isPaintGiayNu || isPaintLien || isPaintNhan || isPaintNgocBoi || isPaintPhu || isPaintWeapon ||
            isPaintStack || isPaintStackLock || isPaintGrocery || isPaintGroceryLock || isPaintUpGrade ||
            isPaintConvert || isPaintUpPearl || isPaintBox || isPaintSplit || isPaintTrade)
            return true;
        return false;
    }
    ...
    public static void setPopupSize(int w, int h)
    {
        ...
        popupX = gW2 - w / 2;
        popupY = gH2 - h / 2;
        if (GameCanvas.isTouch && !isPaintZone && !isPaintTeam && !isPaintClan && !isPaintCharInMap &&
            !isPaintFindTeam && !isPaintFriend && !isPaintEnemies && !isPaintTask && !isPaintMessage)
        {
            if (GameCanvas.h <= 240)
                popupY -= 10;
    ...
    public void paintTitle(mGraphics g, string title, bool arrow)
    {
        ...
        g.fillRoundRect(num - mFont.tahoma_8b.getWidth(title) / 2 - 12, popupY + 4,
            mFont.tahoma_8b.getWidth(title) + 22, 24, 6, 6);
        if ((indexTitle == 0 || GameCanvas.isTouch) && arrow)
        {
            SmallImage.drawSmallImage(g, 989,
                num - mFont.tahoma_8b.getWidth(title) / 2 - 15 - 7 - (GameCanvas.gameTick % 8 <= 3 ? 2 : 0),
                popupY + 16, 2, StaticObj.VCENTER_HCENTER);
            SmallImage.drawSmallImage(g, 989,
                num + mFont.tahoma_8b.getWidth(title) / 2 + 15 + 5 + (GameCanvas.gameTick % 8 <= 3 ? 2 : 0),
        ...
        g.drawRoundRect(num - mFont.tahoma_8b.getWidth(title) / 2 - 12, popupY + 4,
            mFont.tahoma_8b.getWidth(title) + 22, 24, 6, 6);
    ...
    {
        var @char = findCharInMap(playerId);
        if (@char != null)
        {
            if (typePK == 3)
                startYesNoPopUp(info, new Command(mResources.OK, 2000, @char),
                    new Command(mResources.CLOSE, 2009, @char));
            if (typePK == 4)
                startYesNoPopUp(info, new Command(mResources.OK, 2005, @char),
                    new Command(mResources.CLOSE, 2009, @char));
        }
    }
    ...
    public void giaodich(int playerID)
    {
        var @char = findCharInMap(playerID);
        if (@char != null)
            startYesNoPopUp(@char.cName + mResources.want_to_trade, new Command(mResources.YES, 11114, @char),
                new Command(mResources.NO, 2009, @char));
    }
    ...
    public void getFlagImage(int charID, sbyte cflag)
    {
        ...
        if (charID == Char.myCharz().charID)
        {
            ...
            if (Char.myCharz().isGetFlagImage(cflag))
            {
                ...
                for (var i = 0; i < vFlag.size(); i++)
                {
                    var pKFlag = (PKFlag)vFlag.elementAt(i);
        ...
        {
            Res.outz("my cflag: true");
            for (var j = 0; j < vFlag.size(); j++)
            {
                var pKFlag2 = (PKFlag)vFlag.elementAt(j);
    ...
    public void actionPerform(int idAction, object p)
    {
        ...
        switch (idAction)
        {
            ...
            case 11115:
                ...
                {
                    InfoDlg.showWait();
                    Service.gI().playerMenuAction(Char.myCharz().charFocus.charID,
                        (short)Char.myCharz().charFocus.menuSelect);
                }
            ...
            {
                var array2 = (object[])p;
                var skill2 = (Skill)array2[0];
                var num2 = int.Parse((string)array2[1]);
                for (var j = 0; j < onScreenSkill.Length; j++)
            ...
            {
                var array = (object[])p;
                var skill = (Skill)array[0];
                var num = int.Parse((string)array[1]);
                for (var i = 0; i < keySkill.Length; i++)
                    if (keySkill[i] == skill)
                        keySkill[i] = null;
                keySkill[num] = skill;
            ...
            if (idAction != 2)
            {
                switch (idAction)
                {
                    case 11057:
                    {
                        ...
                        var npc = (Npc)p;
                        if (npc.idItem == 0)
                            Service.gI().confirmMenu((short)npc.template.npcTemplateId,
                                (sbyte)GameCanvas.menu.menuSelectedItem);
                        else if (GameCanvas.menu.menuSelectedItem == 0) Service.gI().pickItem(npc.idItem);
                        return;
                    }
                    case 11059:
    ...
    internal void paintGamePad(mGraphics g)
    {
        if (isAnalog != 0 && Char.myCharz().statusMe != 14)
        {
            g.drawImage(keyTouch != 5 && keyMouse != 5 ? imgFire0 : imgFire1, xF + 20, yF + 20,
                mGraphics.HCENTER | mGraphics.VCENTER);
            gamePad.paint(g);
            g.drawImage(keyTouch != 13 ? imgFocus : imgFocus2, xTG + 20, yTG + 20,
                mGraphics.HCENTER | mGraphics.VCENTER);
        }
    }
    ...
    public void paintChatVip(mGraphics g)
    {
        if (vChatVip.size() != 0 && isPaintChatVip)
        {
            ...
            var st = (string)vChatVip.elementAt(0);
    ...
    public void paintPhuBanBar(mGraphics g, int x, int y, int w)
    {
        ...
        var frameHeight = fra_PVE_Bar_0.frameHeight;
        var num = y + frameHeight + mGraphics.getImageHeight(imgBall) / 2 + 2;
        var frameWidth = fra_PVE_Bar_1.frameWidth;
        var num2 = w / 2 - frameWidth / 2;
        var num3 = x - w / 2;
        var num4 = x + frameWidth / 2;
        var y2 = y + 3;
        var num5 = num2 - fra_PVE_Bar_0.frameWidth;
        var num6 = num5 / fra_PVE_Bar_0.frameWidth;
        ...
        if (phuban_Info.pointTeam1 > 0)
        {
            ...
            var num7 = phuban_Info.pointTeam1 * num2 / phuban_Info.maxPoint;
            if (num7 < 0)
                num7 = 0;
            if (num7 > num2)
                num7 = num2;
            ...
            for (var j = 0; j < num6; j++)
                if (j < num6 - 1)
                    fra_PVE_Bar_0.drawFrame(idx2, num3 + fra_PVE_Bar_0.frameWidth + j * fra_PVE_Bar_0.frameWidth, y2, 0,
                        0, g);
                else
        ...
        if (phuban_Info.pointTeam2 > 0)
        {
            ...
            var num8 = phuban_Info.pointTeam2 * num2 / phuban_Info.maxPoint;
            if (num8 < 0)
                num8 = 0;
            if (num8 > num2)
                num8 = num2;
        ...
        var timeCountDown = mSystem.getTimeCountDown(phuban_Info.timeStart, phuban_Info.timeSecond, true, false);
        mFont.tahoma_7b_yellow.drawString(g, timeCountDown, x + 1,
            y + fra_PVE_Bar_1.frameHeight / 2 - mFont.tahoma_7b_green2.getHeight() / 2, 2);
        ...
        if (phuban_Info.type_PB == 0)
            paintChienTruong_Life(g, phuban_Info.maxLife, phuban_Info.color_1, phuban_Info.lifeTeam1, x - 13,
                phuban_Info.color_2, phuban_Info.lifeTeam2, x + 13, num);
    }

    public static void paintChienTruong_Life(mGraphics g, int maxLife, int cl1, int lifeTeam1, int x1, int cl2,
        int lifeTeam2, int x2, int y)
    {
        if (imgBall == null)
            return;
        var num = mGraphics.getImageHeight(imgBall) / 2;
        for (var i = 0; i < maxLife; i++)
        {
            var num2 = 0;
            if (i < lifeTeam1)
                num2 = 1;
            g.drawRegion(imgBall, 0, num2 * num, mGraphics.getImageWidth(imgBall), num, 0, x1 - i * (num + 1), y,
        ...
        for (var j = 0; j < maxLife; j++)
        {
            ...
            if (j < lifeTeam2)
                num3 = 1;
            g.drawRegion(imgBall, 0, num3 * num, mGraphics.getImageWidth(imgBall), num, 0, x2 + j * (num + 1), y,
                mGraphics.VCENTER | mGraphics.HCENTER);
        }
    }
    ...
    public static void addEffectEnd(int type, int subtype, int typePaint, int x, int y, int levelPaint, int dir,
        short timeRemove, Point[] listObj)
    {
        addEffect2Vector(new Effect_End(type, subtype, typePaint, x, y, levelPaint, dir, timeRemove, listObj));
    }

    public static void addEffectEnd_Target(int type, int subtype, int typePaint, Char charUse, Point target,
        int levelPaint, short timeRemove, short range)
    {
        addEffect2Vector(new Effect_End(type, subtype, typePaint, charUse.clone(), target, levelPaint, timeRemove,
            range));
    }
    ...
    internal void paint_xp_bar(mGraphics g)
    {
        ...
        var w = (int)(Char.myCharz().cLevelPercent * GameCanvas.w / 10000);
        g.setColor(16777215);
        g.fillRect(0, GameCanvas.h - 2, w, 2);
        g.setColor(0);
        w = GameCanvas.w / 10;
        for (var i = 1; i < 10; i++) g.fillRect(i * w, GameCanvas.h - 2, 1, 2);
    }
    ...
    public void paint_CT(mGraphics g, int x, int y, int w)
    {
        ...
        if (x < mGraphics.getImageWidth(imgKhung) + w / 2 + 10)
            x = mGraphics.getImageWidth(imgKhung) + w / 2 + 10;
        var num3 = y + fra_PVE_Bar_0.frameHeight + mGraphics.getImageHeight(imgBall) / 2 + 2;
        var frameWidth = fra_PVE_Bar_1.frameWidth;
        var num4 = w / 2 - frameWidth / 2;
        var num5 = x - w / 2 + 3;
        var num6 = x + frameWidth / 2;
        var num7 = y + 3;
        var num8 = num4 - fra_PVE_Bar_0.frameWidth;
        var num9 = num8 / fra_PVE_Bar_0.frameWidth;
        if (num8 % fra_PVE_Bar_0.frameWidth > 0)
            num9++;
        for (var i = 0; i < num9; i++)
        {
            if (i < num9 - 1)
                g.drawRegion(img_ct_bar_0, 0, 15, mGraphics.getImageWidth(img_ct_bar_0), 15, 2,
                    num5 + fra_PVE_Bar_0.frameWidth + i * fra_PVE_Bar_0.frameWidth, num7,
                    mGraphics.TOP | mGraphics.LEFT, true);
            else
                g.drawRegion(img_ct_bar_0, 0, 15, mGraphics.getImageWidth(img_ct_bar_0), 15, 2, num5 + num8, num7,
                    mGraphics.TOP | mGraphics.LEFT, true);
            if (i < num9 - 1)
                g.drawRegion(img_ct_bar_0, 0, 15, mGraphics.getImageWidth(img_ct_bar_0), 15, 2,
                    num6 + i * fra_PVE_Bar_0.frameWidth, num7, mGraphics.TOP | mGraphics.LEFT, true);
            else
                g.drawRegion(img_ct_bar_0, 0, 15, mGraphics.getImageWidth(img_ct_bar_0), 15, 2,
                    num6 + num8 - fra_PVE_Bar_0.frameWidth, num7, mGraphics.TOP | mGraphics.LEFT, true);
        }

        fra_PVE_Bar_0.drawFrame(0, num5, num7, 2, 0, g);
        fra_PVE_Bar_0.drawFrame(0, num6 + num8, num7, 0, 0, g);
        var num10 = nCT_TeamA * 100 / (nCT_nBoyBaller / 2) * num / 100;
        if (num10 > 0)
        {
            if (num10 < 6)
                num10 = 6;
        ...
        if (nCT_TeamA > 0)
            for (var j = 0; j < num2; j++)
                if (j == 0)
                    g.drawRegion(img_ct_bar_0, 0, 60, mGraphics.getImageWidth(img_ct_bar_0), 15, 2, num5, num7,
                        mGraphics.TOP | mGraphics.LEFT, true);
                else
                    g.drawRegion(img_ct_bar_0, 0, 75, mGraphics.getImageWidth(img_ct_bar_0), 15, 2, num5 + j * 6, num7,
                        mGraphics.TOP | mGraphics.LEFT, true);

        GameCanvas.resetTrans(g);
        var num11 = nCT_TeamB * 100 / (nCT_nBoyBaller / 2) * num / 100;
        if (num - (num - num11) > 0)
        {
            if (num11 < 6)
                num11 = 6;
        ...
        if (nCT_TeamB > 0)
            for (var k = 0; k < num2; k++)
                if (k == 0)
                    g.drawRegion(img_ct_bar_0, 0, 30, mGraphics.getImageWidth(img_ct_bar_0), 15, 0, num6 + num8, num7,
                        mGraphics.TOP | mGraphics.LEFT, true);
                else
                    g.drawRegion(img_ct_bar_0, 0, 45, mGraphics.getImageWidth(img_ct_bar_0), 15, 0, num6 + num8 - k * 6,
                        num7, mGraphics.TOP | mGraphics.LEFT, true);

        GameCanvas.resetTrans(g);
        fra_PVE_Bar_1.drawFrame(0, x - frameWidth / 2 + 1, y, 0, 0, g);
        var st = NinjaUtil.getTime((int)((nCT_timeBallte - mSystem.currentTimeMillis()) / 1000)) + string.Empty;
        mFont.tahoma_7b_yellow.drawString(g, st, num5 + w / 2 - 2, y + 5, 2);
        mFont.tahoma_7_grey.drawString(g, "Tầng " + nCT_floor, num5 + w / 2 - 3, y + fra_PVE_Bar_1.frameHeight,
            mFont.CENTER);
        var width = mFont.tahoma_7b_red.getWidth(nCT_TeamA + string.Empty);
        mFont.tahoma_7b_blue.drawString(g, nCT_TeamA + string.Empty, x - frameWidth / 2 - width,
            num7 + fra_PVE_Bar_1.frameHeight, 0);
        SmallImage.drawSmallImage(g, 2325, x - frameWidth / 2 - width - 15, num7 + fra_PVE_Bar_1.frameHeight, 2,
            mGraphics.TOP | mGraphics.LEFT);
        width = mFont.tahoma_7b_red.getWidth(nCT_TeamB + string.Empty);
        mFont.tahoma_7b_red.drawString(g, nCT_TeamB + string.Empty, x + frameWidth / 2,
            num7 + fra_PVE_Bar_1.frameHeight, 0);
        SmallImage.drawSmallImage(g, 2323, x + frameWidth / 2 + width + 3, num7 + fra_PVE_Bar_1.frameHeight, 0,
            mGraphics.TOP | mGraphics.LEFT);
    ...
    internal void paint_board_CT(mGraphics g, int x, int y)
    {
        ...
        {
            var width = mFont.tahoma_7.getWidth("#01 nnnnnnnnnnnn");
            var num = GameCanvas.w - width - 20;
            for (var i = 0; i < nTop; i++)
            {
                var mFont2 = mFont.tahoma_7_white;
                if (i == 0)
                    mFont2 = mFont.tahoma_7_red;
                else if (i == 1)
                    mFont2 = mFont.tahoma_7_yellow;
                else if (i == 2) mFont2 = mFont.tahoma_7_blue;
                if (i == nTop - 1)
                    mFont2 = mFont.tahoma_7_green;
                var array = Res.split((string)res_CT.elementAt(i), "|", 0);
                var array2 = new int[2] { 0, 18 };
                for (var j = 0; j < 2; j++)
                    mFont2.drawString(g, array[j], num + array2[j], y + i * mFont.tahoma_7.getHeight(), 0,
                        mFont.tahoma_7);
            }
        ...
        {
            var s = "#01 namec1000000 0001   00000";
            var array3 = new int[4] { 0, 18, 80, 101 };
            var width2 = mFont.tahoma_7.getWidth(s);
            var num2 = GameCanvas.w - width2 - 20;
            var num3 = y;
            for (var k = 0; k < nTop; k++)
            {
                var array4 = Res.split((string)res_CT.elementAt(k), "|", 0);
                var mFont3 = mFont.tahoma_7_white;
                if (k == 0)
                    mFont3 = mFont.tahoma_7_red;
                else if (k == 1)
                    mFont3 = mFont.tahoma_7_yellow;
                else if (k == 2) mFont3 = mFont.tahoma_7_blue;
                if (k == nTop - 1)
